...
class KVDBPlatformAgentSingleTest extends KVDBPlatformAgentBase {
  ...
  "subscribe" should {
    "find 2 results from publish" ignore {
      val key       = "pub(_)".toLabel
      val resultKey = Results.getKey()
      reset {
        for (e <- writer.subscribe(cnxn)(key)) {
          if (e != None) {
            BasicLogService.tweet(s"SUBSCRIBED = ${e.dispatch}")
          }
        }
      }
      val key1 = """pub("1")""".toLabel
  ...
  "Store" should {
    "not pin cpu at 100% for 1" ignore {
    ...
    "not pin cpu at 100% for 5" ignore {
    ...
    "not pin cpu at 100% for 5 complex keys" ignore {
      ...
      val key2 =
        """disclosedData(fields(id("2222d264-dfe2-43a9-b161-1366439f7bbd"),localeCode("en"),dataClassType("class com.protegra.protunityservices.schema.Role"),connectionType("Basic"),fields("")))""".toLabel
      val key3 =
        """disclosedData(fields(id("3333d264-dfe2-43a9-b161-1366439f7bbd"),localeCode("en"),dataClassType("class com.protegra.protunityservices.schema.Role"),connectionType("Basic"),fields("")))""".toLabel
      val key4 =
        """disclosedData(fields(id("4444d264-dfe2-43a9-b161-1366439f7bbd"),localeCode("en"),dataClassType("class com.protegra.protunityservices.schema.Role"),connectionType("Basic"),fields("")))""".toLabel
      val key5 =
        """disclosedData(fields(id("5555d264-dfe2-43a9-b161-1366439f7bbd"),localeCode("en"),dataClassType("class com.protegra.protunityservices.schema.Role"),connectionType("Basic"),fields("")))""".toLabel
      val value = "not pin cpu 1@protegra.com"
      writer.store(cnxn)(key1, Ground(value))
      writer.store(cnxn)(key2, Ground(value))
      writer.store(cnxn)(key3, Ground(value))
  ...
  "Get" should {
    "not find when key is missing" ignore {
