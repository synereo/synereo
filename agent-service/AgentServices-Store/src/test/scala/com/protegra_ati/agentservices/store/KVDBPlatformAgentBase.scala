...
class KVDBPlatformAgentBase
  ...
  trait CnxnSetup extends Serializable {
  ...
  class WildcardSetup(setupData: (AgentCnxn, String) => Unit) extends Serializable {
  ...
  def testMessaging(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
    ...
    "Persisted Put/Get" should {
      ...
      "work with UUID" ignore new CnxnSetup {
        val key   = s"""testChannel(cachedPutGetUUID("${UUID.randomUUID.toString}"))""".toLabel
    ...
    "Cached Get/Put" should {
      ...
      // TODO: (RACE) see why it fails intermittently
      "consume" ignore new CnxnSetup {
        val testId   = UUID.randomUUID().toString
        val cnxnTest = AgentCnxn(("TestDB" + testId).toURI, "", ("TestDB" + testId).toURI)
        ...
        reset {
          for (e <- reader.get(cnxn)(key)) {
            if (e != None) {
              val result = e.dispatch
              BasicLogService.tweet("cacheGetPutConsume receives a result: " + result)
              Results.saveString(resultKey, result)
            }
          }
        }
    ...
    "Persisted Put/Fetch" should {
      ...
      "respect connection" ignore new CnxnSetup {
      ...
      "not consume" ignore new CnxnSetup {
    ...
    "Cached Fetch/Put" should {
      ...
      "retrieve" ignore new CnxnSetup {
        ...
        reset {
          for (e <- reader.fetch(cnxn)(key)) {
            if (e != None) {
              BasicLogService.tweet("!!!!!!!!!!!!!!!!!!!!!!!!!got into fetch")
              val result = e.dispatch
              Results.saveString(resultKey, result)
            }
          }
      ...
      "consume" ignore new CnxnSetup {
        ...
        BasicLogService.tweet("Sleeping for 300")
        Thread.sleep(TIMEOUT_MED)
        reset { writer.put(cnxn)(key, Ground(value)) }
        BasicLogService.tweet("Sleeping again for 300")
        // SleepToPreventContinuation()
        eventually { Results.savedString(resultKey) must ===(value) }
        fetchMustBe("")(reader, cnxn, key)
      }
    ...
    "Store/Get" should {
      ...
      "retrieve from existing db" ignore new CnxnSetup {
        val cnxnNew = AgentCnxn("Test".toURI, "", "NewDB".toURI)
    ...
    "Put/Delete" should {
      ...
      "delete when found" ignore new CnxnSetup {
      ...
      "not delete when missing " ignore new CnxnSetup {
  ...
  def testWildcardWithPut(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                          reader: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse]) =
    testWildcard(writer, reader, "put", putWildcardData)

  def testWildcardWithStore(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                            reader: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse]) =
    testWildcard(writer, reader, "store", storeWildcardData)

  private def putWildcardData(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                              cnxn: AgentCnxn,
                              value: String) = {
    ...
    reset {
      writer.put(cnxn)(key1.toLabel, Ground(value + "1"))
    }
    reset {
      writer.put(cnxn)(key2.toLabel, Ground(value + "2"))
    }
    reset {
      writer.put(cnxn)(key3.toLabel, Ground(value + "3"))
    }
    reset {
      writer.put(cnxn)(key4.toLabel, Ground(value + "4"))
    }
    reset {
      writer.put(cnxn)(key5.toLabel, Ground(value + "5"))
    }
    reset {
      writer.put(cnxn)(key6.toLabel, Ground(value + "6"))
    }
    reset {
      writer.put(cnxn)(key7.toLabel, Ground(value + "7"))
    }
    reset {
      writer.put(cnxn)(keyEmail.toLabel, Ground(value))
    }
    Thread.sleep(TIMEOUT_LONG)
  }

  //with 28 resolved timeouts can be shortened, still needed it appears
  private def storeWildcardData(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                                cnxn: AgentCnxn,
                                value: String) = {
    val key1     = s"""contentChannel("${UUID.randomUUID.toString}")"""
    val key2     = s"""contentChannel("${UUID.randomUUID.toString}")"""
    val key3     = s"""contentChannel("${UUID.randomUUID.toString}")"""
    val key4     = s"""contentChannel("${UUID.randomUUID.toString}")"""
    val key5     = s"""contentChannel("${UUID.randomUUID.toString}")"""
    val key6     = s"""fakeChannel("${UUID.randomUUID.toString}")"""
    val key7     = s"""fakeChannel("${UUID.randomUUID.toString}")"""
    val keyEmail = """surveyChannel(level1("email"), level2("name"))"""
  ...
  private def testWildcard(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                           ...
                           setupData: (Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse], AgentCnxn,
                                       String) => Unit) = {
    val timeoutBetween: Int = 300
    ...
    "Wildcard Search after " + storageMethod should {
      ...
      // TODO: this fails; looks like a race condition
      "find many values by underscore Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val expectedCollection = setupExpectedResults(value)
        val lblSearch          = "contentChannel(_)".toLabel
        getMustContain(expectedCollection)(reader, cnxnRandom, lblSearch)
      }

      "not find a value by Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "fail(X)".toLabel
        getMustBe("")(reader, cnxnRandom, lblSearch)
      }

      "not find a nested value by Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "abc(xyz(X))".toLabel
        getMustBe("")(reader, cnxnRandom, lblSearch)
      }

      // this is only valid if WildcardSearchShouldNotFind works
      "find a value by Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "surveyChannel(level1(X), level2(Y))".toLabel
        getMustBe(value)(reader, cnxnRandom, lblSearch)
      }

      "find a channel by Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "surveyChannel(X, Y)".toLabel
        getMustBe(value)(reader, cnxnRandom, lblSearch)
      }

      "find a channel by underscore Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "surveyChannel(_, _)".toLabel
        getMustBe(value)(reader, cnxnRandom, lblSearch)
      }

      "find many values by Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val expectedCollection = setupExpectedResults(value)
        val lblSearch          = "contentChannel(X)".toLabel
        fetchMustContain(expectedCollection, resultQ)(reader, cnxnRandom, lblSearch)
      }

      "find many values by underscore Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val expectedCollection = setupExpectedResults(value)
        val lblSearch          = "contentChannel(_)".toLabel
        fetchMustContain(expectedCollection, resultQ)(reader, cnxnRandom, lblSearch)
      }

      "not find a value by Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "fail(X)".toLabel
        fetchMustBe("")(reader, cnxnRandom, lblSearch)
      }

      "not find a nested value by Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "abc(xyz(X))".toLabel
        fetchMustBe("")(reader, cnxnRandom, lblSearch)
      }

      //this is only valid if WildcardSearchShouldNotFind works
      "find a value by Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "surveyChannel(level1(X), level2(Y))".toLabel
        fetchMustBe(value)(reader, cnxnRandom, lblSearch)
      }

      "find a channel by Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "surveyChannel(X, Y)".toLabel
        fetchMustBe(value)(reader, cnxnRandom, lblSearch)
      }

      "find a channel by underscore Fetch" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        val lblSearch = "surveyChannel(_, _)".toLabel
        fetchMustBe(value)(reader, cnxnRandom, lblSearch)
      }
    }
  ...
  def testWildcardWithPutAndCursor(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                                   reader: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse]) =
    testWildcardWithCursor(writer, reader, "put", putWildcardData)

  def testWildcardWithStoreAndCursor(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                                     reader: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse]) =
    testWildcardWithCursor(writer, reader, "store", storeWildcardData)
  ...
  def testWildcardWithCursor(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                             ...
                             setupData: (Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse], AgentCnxn,
                                         String) => Unit) = {
    BasicLogService.tweet("===============testWildcardWithCursor====================")

    "Wildcard Search after store with Cursor" should {
      val timeoutBetween: Int = 300

      // TODO: (RACE) see why it fails
      s"$storageMethod: find many results by Get" ignore new CnxnSetup {
        getCountMustBe(5)(reader, cnxnRandom, "contentChannel(_)")
      }

      s":$storageMethod: not find a value by Get" ignore new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        BasicLogService.tweet("===============start of find many results by ====================")
        getCountMustBe(0)(reader, cnxnRandom, "fail(X)")
      }

      s"$storageMethod: not find a nested value by Get" in new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        getCountMustBe(0)(reader, cnxnRandom, "abc(xyz(X))")
      }

      s"$storageMethod: find many results by Fetch" ignore new CnxnSetup {
        countMustBe(5)(reader, cnxnRandom, "contentChannel(_)")
      }

      s"$storageMethod: not find a value by Fetch" in new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        countMustBe(0)(reader, cnxnRandom, "fail(X)")
      }

      s"$storageMethod: not find a nested value by Fetch" in new WildcardSetup(setupData(writer, _: AgentCnxn, _: String)) {
        countMustBe(0)(reader, cnxnRandom, "abc(xyz(X))")
      }
    }
  }

  def testWildcardWithCursorBefore(writer: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse],
                                   reader: Being.AgentKVDBNode[PersistedKVDBNodeRequest, PersistedKVDBNodeResponse]) = {
    "Wildcard Search before put with Cursor" should {
      ...
      "find many results by Get" ignore new CnxnSetup {
        val key           = "getCountChannel(_)".toLabel
        val RESULT_PREFIX = "R"
        val resultKey     = RESULT_PREFIX + Results.getKey()

        reset {
          for (c <- reader.get(cursor = true)(cnxnRandom)(key)) {
            if (c.isDefined) {
              var found = 0
              for (e <- c.dispatchCursor) {
                if (e != None) {
                  val result = e.dispatch
                  BasicLogService.tweet(s"*************getCount - received : $result")
                  found += 1
                  BasicLogService.tweet(s"*************getCount - found : $found")
                }
              }
              BasicLogService.tweet(s"*************getCount - found : $found")
        ...
        // SleepToPreventContinuation()
        // fetchString(_resultsQ, cnxnTest, resultKey) must ===("5").eventually(10, TIMEOUT_EVENTUALLY)
        eventually { Results.savedString(resultKey, 3) must ===("5") }
      }

      "find many results by Fetch" ignore new CnxnSetup {
        val key           = "fetchCountChannel(_)".toLabel
        val RESULT_PREFIX = "R"
        val resultKey     = RESULT_PREFIX + Results.getKey()

        reset {
          for (c <- reader.fetch(cursor = true)(cnxnRandom)(key)) {
            if (c.isDefined) {
              var found = 0
              for (e <- c.dispatchCursor) {
                if (e != None) {
                  val result = e.dispatch
                  BasicLogService.tweet(s"*************fetchCount - received : $result")
                  found += 1
                  BasicLogService.tweet(s"*************fetchCount - found : $found")
                }
              }
              BasicLogService.tweet(s"*************fetchCount - found : $found")
              Results.saveString(resultKey, found.toString)
        ...
        putWildcardData(writer, cnxnRandom, value)
        // SleepToPreventContinuation()
        // fetchString(reader, cnxnTest, resultKey) must ===("5").eventually(10, TIMEOUT_EVENTUALLY)
        eventually { Results.savedString(resultKey, 3) must ===("5") }
      }
    }
  }
