...
object BFactoryCommLink
       extends PersistedMonadicKVDBMongoNodeScope[String,String,String,ConcreteBFactHL.BFactHLExpr]
       with UUIDOps
  with Serializable
{
...
//                       new XStream( new JettisonMappedXmlDriver() )
            ...
            override def asResource(
              ...
              BasicLogService.tweet(
                  (
                    "BFactoryCommLink -- PersistedMonadicKVDB : "
                    + "\nmethod : asResource "
                    + "\nthis : " + this
                    + "\nkey : " + key
                    + "\nvalue : " + value
...
trait BFactoryCommLinkConfiguration {
  self : EvalConfig =>
  def clientHostName() : String = {
    try {
      evalConfig().getString( "BFactoryCommLinkClientHost" )
    }
    catch {
  ...
  def clientPort() : Int = {
    try {
      evalConfig().getInt( "BFactoryCommLinkClientPort" )
    }
    catch {
  ...
  def serverHostName() : String = {
    try {
      evalConfig().getString( "BFactoryCommLinkServerHost" )
    } 
    catch {
      case e : Throwable => "localhost"
    }
  ...
  def serverPort() : Int = {
    try {
      evalConfig().getInt( "BFactoryCommLinkServerPort" )
    }
    catch {
      case e : Throwable => 5672
    }
...
object BFactoryCommLinkCtor extends EvalConfig
