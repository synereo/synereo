        ...
        override def mkCache[ReqBody <: PersistedKVDBNodeRequest, RspBody <: PersistedKVDBNodeResponse]( 
          ...
          new AgentKVDB[ReqBody, RspBody](
            ...
            extends MongoDBManifest( /* database */ ) {
              ...
              def asCacheValue(ccl: CnxnCtxtLabel[String, String, String]): String = ccl match {
                case CnxnCtxtBranch("string", CnxnCtxtLeaf(Left(rv)) :: Nil) =>
                  fromXQSafeJSONBlob(rv) match {
                    case rsrc: mTT.Resource => getGV(rsrc).getOrElse("")
                  }
                    ...
                    matchMap( key, k ) match {
                      case Some( soln ) =>
                        if (compareNameSpace(ns, kvNameSpace)) {
                          ...
                          emT.PlaceInstance(k, Left[mTT.Resource, List[Option[mTT.Resource] => Unit@suspendable]](boundWrapper), theEMTypes.PrologSubstitution(soln).asInstanceOf[emT.Substitution])
...
//                                 new ObjectInputStream( new ByteArrayInputStream(  data ) )
    ...
    def handleEntry( json : JValue, acc : Buffer[Elem] ) : Buffer[Elem] = {
      handleEntry(
        ...
        "com.protegra.agentservicestore.usage.AgentKVDBMongoScope$TheMTT"
      )(
        json, acc
      )
    }
...
//     import org.basex.core.cmd.CreateDB
    ...
    = {
      ...
      val lthrd = AgentKVDBMongoScope.getUUID + ""

      AgentKVDBMongoScope.loadData( numOfEntries )
      // for(   
...
//       }
    ...
    def cnxnStream[ReqBody <: PersistedKVDBNodeRequest, RspBody <: PersistedKVDBNodeResponse](
      ...
      def fresh( seed : acT.AgentCnxn ) : acT.AgentCnxn = {
        ...
        val cnxnPtn = "cnxn_"
        val cnxnId = AgentKVDBMongoScope.getUUID
        val lthrd = cnxnId + ""
