...
class StorableDataTest extends SpecificationWithJUnit
{
  ...
  "store key" should {
    ...
    "generate store key correctly for a Profile" in {
      ...
      val expectedStoreKey = "profile(data(" + KEYS + "(id(\"MY_ID\"),localeCode(\"en\"),recVerNum(\"1\")),fields(firstName(\"FirstName\"),lastName(\"LastName\"),description(\"desc \"),emailAddress(\"123456789 test com\"),country(\"CA\"),region(\"someCAprovince\"),city(\"city\"),postalCode(\"postalCode\"),website(\"website\"),image(\"\"))))"
