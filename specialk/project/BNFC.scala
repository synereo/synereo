...
object BNFC {
  ...
  lazy val bnfcClean        = taskKey[Unit]("Cleans BNFC-generated Java files")
  lazy val bnfcGenerate     = taskKey[Unit]("Generates Java files from BNFC grammar files")
  lazy val bnfcCleanDocs    = taskKey[Unit]("Cleans BNFC-generated LaTeX files")
  lazy val bnfcGenerateDocs = taskKey[Unit]("Generates LaTeX files from BNFC grammar files")
  ...
  lazy val bnfcSettings = Seq(
    bnfcNamespace    := "com.biosimilarity.lift.model",
    ...
    bnfcClean        := cleanDir(bnfcOutputDir.value / nsToPath(bnfcNamespace.value)),
    bnfcGenerate     := bnfcFiles(bnfcGrammarDir.value).foreach { (f: File) =>
      bnfcGenerateSources((fullClasspath in Compile).value, f, bnfcOutputDir.value, bnfcNamespace.value)
    },
    bnfcCleanDocs    := cleanDir(bnfcDocDir.value),
    bnfcGenerateDocs := bnfcFiles(bnfcGrammarDir.value).foreach { (f: File) =>
