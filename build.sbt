...
lazy val casperDepsSettings = Seq(
  resolvers ++= additionalResolvers,
  libraryDependencies ++= casperDeps ++ Seq(
    "org.scala-lang" % "scala-actors"  % scalaVersion.value,
    "org.scala-lang" % "scala-reflect" % scalaVersion.value))

lazy val casperSettings = Seq(
  autoCompilerPlugins := true,
  addCompilerPlugin("org.scala-lang.plugins" % "continuations" % "2.10.5"),
  name := "agentservices-store-ia",
  organization := "com.protegra-ati",
...
lazy val casper = (project in file("casper"))
  .settings(casperSettings: _*)
  .settings(casperDepsSettings: _*)
  .settings(bnfcSettings: _*)
  .dependsOn(specialk)
...
lazy val root = (project in file("."))
  .aggregate(specialk, agentService, casper, gloseval)
  .dependsOn(specialk, agentService, casper, gloseval)
