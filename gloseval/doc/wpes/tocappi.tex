...
\normalsize{ 
  ...
  rossopoulou and Nolan argue persuasively for the need for a means
...
\begin{eqnarray*}
	& x{(}{z}{)} \Rightarrow {w}{!}{(}{y}{!}{(}{*}{z}{)}{)} \juxtap x{!}{(}{P}{)} & \\
	\red 
	& {w}{!}{(}{y}{!}{(}P{)}{)} & \\
\end{eqnarray*}
...
We illustrate the point with two gadgets that make ECMAScript easier
to interpret in \rhoc. The first is a mutable single-place cell for
...
Notice that the cell's access to $\texttt{state}$ is kept in a private channel
$\texttt{v}$. The external API is through $\texttt{slot}$. When translating
\texttt{Javascript} ocap examples to \rhoc\; we will effectively treat
the state of an object as a parallel composition of cells.
...
We will iterate this basic idea to build up to modeling the bank
...
Focus on deniability. The core idea is to use rely-guarantee combined with the adjunct to spatial separation.
