...
\begin{lstlisting}[mathescape]
def $Map( slot, key_1, state_1, \ldots , key_n, state_n ) \Rightarrow$ {
  new $( v_1, \ldots v_n)$ {
    $v_1!( state_1 )$
    $\ldots$
    $v_n!( state_n )$
    $slot ? get( key_1, ret ) \Rightarrow$ { 
      $v_1?( x ) \Rightarrow ret!( x )$
      $Map( slot, key_1, state_1, \ldots , key_n, state_n )$
    }
    $\ldots$
    $slot ? get( key_n, ret ) \Rightarrow$ { 
...
basic patterns we can already reason about policy.  The shape of the
...
\begin{eqnarray*}
 \meaningof{\texttt{var}\; x; P}( k ) & \defneqls & \binpar{Cell( x, \texttt{undefined} )}{\meaningof{P}(k)} \\
 \meaningof{\texttt{var}\; x = v; P}( k ) & \defneqls & \texttt{new}(k_1)\{ \binpar{Cell( x, \meaningof{ v }(x, k_1 ) )}{k_1?() \Rightarrow \meaningof{P}(k)}\} \\
 ...
 \meaningof{\texttt{def}(\{key_1: state_1, \ldots, key_n: state_n\})}(x, k) & \defneqls & Map(x, key_1, state_1, \ldots, key_n, state_n) | k!() \\
\end{eqnarray*}

We do not give an explicit translation of {\tt WeakMap} because it is quite
large, but it's essentially the same as $Map$ plus some machinery for performing
updates.
...
translate the code fragment below we calculate directly
...
inserted bugs \cite[Section 7]{Yee:EECS-2007-136}. 
...
cell. In the paragraphs above we witnessed a basic form of
argumentation: if $Q \models \phi$ then when $P \models \psi$ we have
that $\texttt{new}(x_1,\ldots,x_N)\binpar{P}{Q} \wbbisim S$ for some
properties $\phi$ and $\psi$ and some behavior, $S$. In light of
theorem 3.0.1 we can transform this into a statement of the form if $Q
\models \phi$ then when $P \models \psi$ we have that
$\texttt{new}(x_1,\ldots,x_N)\binpar{P}{Q} \models \omega$ for some
properties $\phi$,$\psi$, and $\omega$. Notice that the form of the
statement remains true to the intent of capturing deniability even if
the logic is \emph{more} discriminating than bisimulation -- which is
the case if we include the separation operator (as noted in
\cite{DBLP:conf/fossacs/Caires04}). This shape turns out to be
precisely that of rely-guarantee identified by Honda in
\cite{Honda:2008:UTP:2227536.2227558} and, as Honda observes, can be
internalized in the logic as the adjunct to separation. 

Thus, we include formulae of the form $\phi \rhd \psi$ with semantics $\pmeaningof{\phi \rhd \psi}( v ) = \{ P : \forall P_0. P_0 \in \pmeaningof{\phi}( v ) \Rightarrow \binpar{P_0}{P}, \; P_0 \in \pmeaningof{\psi}(v), \;  P_1 \in \pmeaningof{\psi}(v) \}$
