...
request to set any of the slots it contains. With these two patterns
...
reason about policy.  The shape of the reason about policy for these
...
separation this shape turns out to be precisely that of rely-guarantee
...
separation. Thus, we include rely-guarantee formulae, $\phi \rhd \psi$
with semantics $\pmeaningof{\phi \rhd \psi}( v ) = \{ P : \forall Q. Q
\in \pmeaningof{\phi}( v ) \Rightarrow \binpar{P}{Q}\in
\pmeaningof{\psi}(v) \}$, as the logical form that mediates between
closed-world and open-world formulation of deniability.

To apply this to the bank example amounts to recognizing that the
\texttt{purse} is essentially a $Cell$ with $slot$, \texttt{purse},
the internal state of which is \texttt{balance} and that \texttt{mint}
is simply a cell factory. If a process $Q$ is walled off from
\texttt{purse},
i.e. $\pgfp{X}{\pprefix{\pquotep{\texttt{purse}}}{b}{X} \& \neg
  \pprefix{\pquotep{\neg \texttt{purse}}}{b}{X}}$, then it cannot
affect \texttt{balance}, $\pgfp{X}{\neg
  \pprefix{\pquotep{balance}}{b}{X}}$. In short,
$Cell(\texttt{purse},initBal) \models
\pgfp{X}{\pprefix{\pquotep{\texttt{purse}}}{b}{X} \& \neg
  \pprefix{\pquotep{\neg \texttt{purse}}}{b}{X}} \rhd \pgfp{X}{\neg
  \pprefix{\pquotep{balance}}{b}{X}}$.
