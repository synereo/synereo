...
class OmniCoreStubServer extends RPCConfiguration with Serializable {
  ...
  def start() = {
    ...
    WireMock.stubFor(post(anyUrl())
      .withBasicAuth(rpcUser, rpcPassword)
      .withRequestBody(matchingJsonPath("$[?(@.method == 'listproperties_MP')]"))
      .willReturn(aResponse().withBody(RPC_RESPONSE_LISTPROPERTIES)))
