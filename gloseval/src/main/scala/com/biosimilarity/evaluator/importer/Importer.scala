...
object Importer extends EvalConfig
  ...
  def createAgent(agent: AgentDesc): Option[String] = {
    ...
    srpClient.init
    val r1 = parse(glosevalPost(Api.CreateUserStep1Request(eml))).extract[ApiResponse]
    r1.responseContent match {
      ...
      case CreateUserStep1Response(salt) =>
        ...
        val r2 = parse(glosevalPost(Api.CreateUserStep2Request(eml, salt, srpClient.generateVerifier, jsonBlob))).extract[ApiResponse]
        r2.responseContent match {
          case ApiError(reason) =>
            println(s"create user, step 2, failed, reason : $reason")
            None
          case CreateUserWaiting(tok) =>
            val r3 = parse(glosevalPost(Api.ConfirmEmailToken(tok))).extract[ApiResponse]
